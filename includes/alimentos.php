...
// Para cada alimento no banco, mostra uma checkbox com esses valores
foreach ($conn->query(" SELECT alimento_id, nome FROM alimento" , PDO::FETCH_ASSOC) as $alimento){
        ...
        echo !$aux ? '<input class="form-check-input " type="checkbox" id="'. $alimento['alimento_id'] .'" value="' . $alimento['alimento_id']. '"' . ($alimentos[$a]["alimento_id"]==1 ? 'checked' : '') . ' name="alimentos[]">  <label class="form-check-label" for="' . $alimento['alimento_id'] .'" >' . $alimento['nome'] : $alimento['nome'] . '</label>' ;
        echo ('</div>');
